    ...
    for (var ids in couponIds) {
      PromotionRequest.promotionMgicData(ids, 1).then((value) {
        ...
        for (var item in value["list"]) {
          CouponItemDataModel temp = CouponItemDataModel.fromJson(Map.from(item));
          list.add(temp);
        }
        coupons[ids] = list;
      }).whenComplete(() {
    ...
    return ScrollablePositionedList.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (model.type == "title") {
          ...
          return Container(
            height: model.couponConfig.gridHeight(coupons[model.id.toString()].length, context),
            color: Colors.yellow,
            child: CouponItems(itemConfigModel: model.couponConfig, dataList: coupons[model.id.toString()]),
          );
        } else {
          return GestureDetector(
            child: Container(
